...
// Collision
//--------------------------------------------------------------------------------------------------------
Collision::Collision(ColliderComponent* newFirst, ColliderComponent* newSecond,
                     sf::FloatRect newCollisionRect)
    : m_first(newFirst),
      m_second(newSecond),
      m_collisionRect(newCollisionRect) {
  FE_APP_TRACE("Collision object created between: " +
               m_first->GetGameObject()->GetName() + " and " +
               m_second->GetGameObject()->GetName());
}
//--------------------------------------------------------------------------------------------------------
Vector2Df Collision::GetKnockbackDirection() const {
  auto transformA =
      m_first->GetGameObject()->GetComponent<TransformComponent>();
  auto transformB =
      m_second->GetGameObject()->GetComponent<TransformComponent>();

  if (!transformA || !transformB) {
    FE_CORE_ERROR(
        "Collision::GetKnockbackDirection: One of the GameObjects is missing a "
        "TransformComponent!");
    return {0.0f, 0.0f};
  }
