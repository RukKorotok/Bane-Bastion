...
void PhysicsSystem::Update() {
  for (size_t i = 0; i < m_colliders.size(); i++) {
    ...
    auto body =
        m_colliders[i]->GetGameObject()->GetComponent<RigidbodyComponent>();
    ...
    for (size_t j = 0; j < m_colliders.size(); j++) {
      ...
      if (m_colliders[i]->m_bounds.intersects(m_colliders[j]->m_bounds,
                                              intersection)) {
        if (m_colliders[i]->m_isTrigger || m_colliders[j]->m_isTrigger) {
          if (m_triggersEnteredPair.find(m_colliders[i]) ==
              m_triggersEnteredPair.end()) {
            ...
            FE_APP_TRACE(
                "Trigger Enter: " + m_colliders[i]->GetGameObject()->GetName() +
          ...
          Vector2Df aCenter(m_colliders[i]->m_bounds.left +
                                m_colliders[i]->m_bounds.width / 2.f,
                            m_colliders[i]->m_bounds.top +
                                m_colliders[i]->m_bounds.height / 2.f);
          Vector2Df bCenter(m_colliders[j]->m_bounds.left +
                                m_colliders[j]->m_bounds.width / 2.f,
                            m_colliders[j]->m_bounds.top +
                                m_colliders[j]->m_bounds.height / 2.f);

          auto aTransform = m_colliders[i]
                                ->GetGameObject()
          ...
          if (intersectionWidth > intersectionHeight) {
            if (aCenter.y < bCenter.y) {
              ...
              FE_APP_TRACE("Collision: " +
                           m_colliders[i]->GetGameObject()->GetName() + " TOP");
            } else {
              aTransform->MoveBy({0.f, intersectionHeight});
              FE_APP_TRACE(
                  "Collision: " + m_colliders[i]->GetGameObject()->GetName() +
                  " DOWN");
            }
          } else {
  ...
  for (auto it = m_triggersEnteredPair.begin();
       it != m_triggersEnteredPair.end();) {
...
void PhysicsSystem::Subscribe(ColliderComponent* collider) {
  ...
  FE_APP_TRACE("PhysicsSystem: Collider subscribed (" +
