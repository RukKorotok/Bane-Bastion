	...
	void PhysicsSystem::Update()
	{
		for (int i = 0; i < colliders.size(); i++)
		{
			...
			for (int j = 0; j < colliders.size(); j++)
			{
				...
				if (colliders[i]->m_bounds.intersects(colliders[j]->m_bounds, intersection))
				{
					if (colliders[i]->m_isTrigger != colliders[j]->m_isTrigger)
					{
					...
					else if (!colliders[i]->m_isTrigger)
					{
						...
						Vector2Df aPosition = { colliders[i]->m_bounds.left,  colliders[i]->m_bounds.top };
