...
void InitSprite(sf::Sprite& sprite, float desiredWidth, float desiredHeight,
  ...
  FE_APP_TRACE("Sprite initialized: Size " + std::to_string(desiredWidth) +
               "x" + std::to_string(desiredHeight));
}
//-----------------------------------------------------------------------------------------------------------
...
sf::Vector2f GetRandomSpritePositionInRectangle(const sf::Sprite& sprite,
  ...
  if (rect.width <= bounds.width * 2 || rect.height <= bounds.height * 2) {
    FE_CORE_WARN(
        "GetRandomSpritePosition: Target rectangle is too small for the "
        "sprite. Returning rect center.");
...
void SetSpriteSize(sf::Sprite& sprite, float desiredWidth,
  ...
  if (localRect.width > 0 && localRect.height > 0) {
    sprite.setScale(desiredWidth / localRect.width,
                    desiredHeight / localRect.height);
  } else {
    FE_CORE_ERROR(
        "SetSpriteSize: Sprite has no texture or empty local bounds!");
  }
}
//-----------------------------------------------------------------------------------------------------------
...
bool CheckSpriteIntersection(const sf::Sprite& sprite1,
  ...
  float distanceSq = (pos1.x - pos2.x) * (pos1.x - pos2.x) +
