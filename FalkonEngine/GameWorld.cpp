...
void GameWorld::LateUpdate() {
  if (!m_markedToDestroyGameObjects.empty()) {
    FE_APP_TRACE("LateUpdate: Destroying " +
                 std::to_string(m_markedToDestroyGameObjects.size()) +
                 " marked objects.");

    for (int i = static_cast<int>(m_markedToDestroyGameObjects.size()) - 1;
         i >= 0; i--) {
...
void GameWorld::DestroyGameObject(GameObject* gameObject) {
  ...
  auto it = std::find(m_markedToDestroyGameObjects.begin(),
                      m_markedToDestroyGameObjects.end(), gameObject);

  if (it == m_markedToDestroyGameObjects.end()) {
    m_markedToDestroyGameObjects.push_back(gameObject);
    FE_APP_TRACE("GameObject '" + gameObject->GetName() +
...
void GameWorld::DestroyGameObjectImmediate(GameObject* gameObject) {
  ...
  FE_APP_TRACE("Immediate destruction of '" + objName + "' and its " +
               std::to_string(allTransforms.size() - 1) + " children.");
  ...
  for (auto* t : allTransforms) {
    ...
    m_gameObjects.erase(
        std::remove(m_gameObjects.begin(), m_gameObjects.end(), obj),
        m_gameObjects.end());
    m_markedToDestroyGameObjects.erase(
        std::remove(m_markedToDestroyGameObjects.begin(),
                    m_markedToDestroyGameObjects.end(), obj),
