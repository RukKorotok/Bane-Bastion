...
SpriteColliderComponent::SpriteColliderComponent(GameObject* gameObject)
  ...
  FE_CORE_ASSERT(
      m_sprite != nullptr,
      "SpriteRenderer on " + p_gameObject->GetName() + " has no SFML Sprite!");

  PhysicsSystem::Instance()->Subscribe(this);

  FE_APP_TRACE("SpriteColliderComponent initialized for: " +
               p_gameObject->GetName());
}
//-----------------------------------------------------------------------------------------------------------
SpriteColliderComponent::~SpriteColliderComponent() {
  PhysicsSystem::Instance()->Unsubscribe(this);
  FE_APP_TRACE("SpriteColliderComponent destroyed for: " +
               p_gameObject->GetName());
}
//-----------------------------------------------------------------------------------------------------------
...
Vector2Df SpriteColliderComponent::GetSizeInUnits(float ppu) const {
  if (ppu <= 0.0f) {
    FE_CORE_WARN(
        "GetSizeInUnits: ppu must be greater than zero. Setting to 1.0f");
    ppu = 1.0f;
  }
