...
void TransformComponent::SetParent(TransformComponent* newParent) {
  if (newParent == this || m_parent == newParent) {
    FE_CORE_ASSERT(newParent != this,
                   "Circular dependency: Transform cannot be its own parent!---" + p_gameObject->GetName());
    return;
  }
  ...
  if (m_parent == nullptr && newParent != nullptr) {
    ...
    newParent->p_gameObject->AddChild(p_gameObject);
    m_localTransform =
        newParent->GetWorldTransform().GetInversed() * (m_parent->GetWorldTransform() * m_localTransform);
  }
...
void TransformComponent::setLocalInfoFrom(const Matrix2D& transform) const {
  ...
  m_localScale.x = std::sqrt(transform.GetMatrix()[0][0] * transform.GetMatrix()[0][0] +
                             transform.GetMatrix()[1][0] * transform.GetMatrix()[1][0]);
  m_localScale.y = std::sqrt(transform.GetMatrix()[0][1] * transform.GetMatrix()[0][1] +
                             transform.GetMatrix()[1][1] * transform.GetMatrix()[1][1]);

  m_localRotation =
      static_cast<float>(std::atan2(transform.GetMatrix()[0][1], transform.GetMatrix()[0][0]) * rad_to_deg);
}
//-----------------------------------------------------------------------------------------------------------
...
void TransformComponent::Invalidate() {
  // Если объект уже помечен как грязный, его детей помечать второй раз не нужно
  // Это важная оптимизация для больших деревьев UI
  if (m_isUpdated) {
    m_isUpdated = false;

    // Используем стрелочку для указателя
    for (auto* childGO : p_gameObject->GetChildren()) {
      if (childGO) {
        auto* childTransform = childGO->GetComponent<TransformComponent>();
        if (childTransform) {
