...
TransformComponent::TransformComponent(GameObject* gameObject)
  ...
  FE_CORE_ASSERT(p_gameObject != nullptr,
                 "TransformComponent created without a valid GameObject!");
}
//-----------------------------------------------------------------------------------------------------------
...
void TransformComponent::SetWorldPosition(float positionX, float positionY) {
  ...
  if (m_parent == nullptr) {
    ...
    FE_CORE_ASSERT(std::abs(newWorldTransform.Determinant()) > 0.0001f,
                   "Parent transform is non-invertible (Scale might be 0)!");

    m_localTransform =
        m_parent->GetWorldTransform().GetInversed() * newWorldTransform;
...
void TransformComponent::ScaleBy(float scaleX, float scaleY) {
  if (std::abs(scaleX) < 0.0001f || std::abs(scaleY) < 0.0001f) {
    FE_CORE_WARN("Attempted to scale by near-zero value on: " +
...
void TransformComponent::SetParent(TransformComponent* newParent) {
  if (newParent == this) {
    FE_CORE_ASSERT(
        newParent != this,
        "Circular dependency: Transform cannot be its own parent!---" +
            p_gameObject->GetName());
    return;
  }

  FE_CORE_INFO(
      "Changing parent for: " + p_gameObject->GetName() +
  ...
  if (m_parent == nullptr && newParent != nullptr) {
    ...
    m_localTransform =
        newParent->GetWorldTransform().GetInversed() * m_localTransform;
    ...
    m_parent->p_gameObject->RemoveChild(p_gameObject);
    newParent->p_gameObject->AddChild(p_gameObject);
    m_localTransform = newParent->GetWorldTransform().GetInversed() *
                       (m_parent->GetWorldTransform() * m_localTransform);
  }
...
void TransformComponent::Print() const {
  ...
  msg += "\n  Pos World: (" + std::to_string(GetWorldPosition().x) + ", " +
         std::to_string(GetWorldPosition().y) + ")";
  msg += "\n  Rot World: " + std::to_string(GetWorldRotation());
  msg += "\n  Scale World: (" + std::to_string(GetWorldScale().x) + ", " +
         std::to_string(GetWorldScale().y) + ")";
...
void TransformComponent::setWorldInfoFrom(const Matrix2D& transform) const {
  ...
  m_rotation =
      static_cast<float>(std::atan2(matrix[0][1], matrix[0][0]) * RAD_TO_DEG);
}
//-----------------------------------------------------------------------------------------------------------
void TransformComponent::setLocalInfoFrom(const Matrix2D& transform) const {
  ...
  m_localRotation = static_cast<float>(
...
void TransformComponent::updateLocalTransform() const {
  if (!m_isUpdated) {
    m_localTransform =
        createTransform(m_localPosition, m_localRotation, m_localScale);
    m_isUpdated = true;
  }
}
//-----------------------------------------------------------------------------------------------------------
