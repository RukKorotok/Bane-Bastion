...
SpriteRendererComponent::SpriteRendererComponent(GameObject* gameObject)
  ...
  FE_CORE_ASSERT(m_transform != nullptr,
                 "SpriteRendererComponent added to GameObject '" +
                     p_gameObject->GetName() + "' without TransformComponent!");

  FE_APP_TRACE("SpriteRendererComponent initialized for: " +
...
SpriteRendererComponent::~SpriteRendererComponent() {
  ...
  FE_APP_TRACE("SpriteRendererComponent destroyed for: " +
               p_gameObject->GetName());
}
//-----------------------------------------------------------------------------------------------------------
...
void SpriteRendererComponent::Render() {
  if (m_sprite != nullptr && m_sprite->getTexture() != nullptr) {
    FE_CORE_ASSERT(m_transform != nullptr,
                   "Transform lost in SpriteRenderer::Render!");

    m_sprite->setPosition(
        Convert<sf::Vector2f, Vector2Df>(m_transform->GetWorldPosition()));
    m_sprite->setRotation(m_transform->GetWorldRotation());

    Vector2Df transformScale = m_transform->GetWorldScale();

    m_sprite->setScale(
        {m_scale.x * transformScale.x, m_scale.y * transformScale.y});
    ...
    if (lastWarnedObject != p_gameObject->GetName()) {
      FE_CORE_WARN("SpriteRenderer on '" + p_gameObject->GetName() +
                   "' has no texture assigned. Skipping render.");
...
void SpriteRendererComponent::SetTexture(const sf::Texture& newTexture) {
  ...
  FE_CORE_INFO("Texture set for '" + p_gameObject->GetName() +
               "'. Size: " + std::to_string(textureSize.x) + "x" +
               std::to_string(textureSize.y));
}
//-----------------------------------------------------------------------------------------------------------
void SpriteRendererComponent::SetPixelSize(int newWidth, int newHeight) {
  if (m_sprite == nullptr || m_sprite->getTexture() == nullptr) {
    FE_CORE_ERROR("Cannot SetPixelSize: Sprite or Texture is null on " +
                  p_gameObject->GetName());
    return;
  }
  ...
  if (originalSize.x == 0 || originalSize.y == 0) {
    FE_CORE_ERROR("SetPixelSize failed: Texture size is zero on " +
                  p_gameObject->GetName());
    return;
  }

  m_scale = {(float)newWidth / (float)originalSize.x,
