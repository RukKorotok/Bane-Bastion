...
void ResourceSystem::LoadTexture(const std::string& name,
  ...
  if (newTexture->loadFromFile(sourcePath)) {
    ...
    FE_CORE_INFO("ResourceSystem: Texture loaded [" + name + "] from " +
                 sourcePath);
  } else {
  ...
  if (textureMap.loadFromFile(sourcePath)) {
    ...
    if (sourceImage.loadFromFile(sourcePath)) {
      ...
      if (elementPixelSize.x == 0 || elementPixelSize.y == 0) {
        FE_CORE_ERROR("ResourceSystem: Invalid element size for TextureMap " +
                      name);
        return;
      }

      for (unsigned int y = 0; y <= imageSize.y - elementPixelSize.y &&
                               loadedElements < totalElements;
           y += elementPixelSize.y) {
        for (unsigned int x = 0; x <= imageSize.x - elementPixelSize.x &&
                                 loadedElements < totalElements;
             x += elementPixelSize.x) {
          sf::Texture* newElement = new sf::Texture();

          if (newElement->loadFromImage(
                  sourceImage,
                  sf::IntRect(x, y, elementPixelSize.x, elementPixelSize.y))) {
      ...
      m_textureMaps.emplace(name, textureMapElements);
      FE_CORE_INFO("ResourceSystem: TextureMap loaded [" + name +
                   "]. Elements: " + std::to_string(textureMapElements.size()));
    } else {
      FE_CORE_ERROR("ResourceSystem: Failed to load TextureMap from " +
                    sourcePath);
    }
  }
}
...
sf::Texture* ResourceSystem::GetTextureMapElementCopy(const std::string& name,
  ...
  if (it == m_textureMaps.end()) {
    FE_CORE_ERROR(
        "ResourceSystem: Cannot copy element. TextureMap not found: " + name);
    return nullptr;
  }
  ...
  if (textures[elementIndex] == nullptr) {
    FE_CORE_ERROR("ResourceSystem: Texture at index " +
                  std::to_string(elementIndex) + " in Map '" + name +
                  "' is null!");
    return nullptr;
  }

  FE_APP_TRACE("ResourceSystem: Created copy of TextureMap element [" + name +
...
// Music system
//-----------------------------------------------------------------------------------------------------------
sf::Music* ResourceSystem::LoadMusic(const std::string& name,
                                     const std::string& sourcePath) {
