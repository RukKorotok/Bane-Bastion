...
RigidbodyComponent::RigidbodyComponent(GameObject* gameObject)
  ...
  FE_CORE_ASSERT(m_transform != nullptr,
                 "RigidbodyComponent added to GameObject '" +
                     p_gameObject->GetName() + "' without TransformComponent!");

  FE_APP_TRACE("RigidbodyComponent initialized for: " +
...
void RigidbodyComponent::SetLinearVelocity(Vector2Df newLinearVelocity) {
  if (!std::isfinite(newLinearVelocity.x) ||
      !std::isfinite(newLinearVelocity.y)) {
    FE_CORE_ERROR("Attempted to set non-finite LinearVelocity on: " +
                  p_gameObject->GetName());
    return;
  }
...
void RigidbodyComponent::SetAngleVelocity(float newAngleVelocity) {
  if (!std::isfinite(newAngleVelocity)) {
    FE_CORE_ERROR("Attempted to set non-finite AngleVelocity on: " +
                  p_gameObject->GetName());
    return;
  }
...
void RigidbodyComponent::ApplyImpulse(Vector2Df direction, float force) {
  ...
  if (lenSq > 0.000001f) {
    ...
    FE_CORE_WARN("ApplyImpulse called with zero-length direction on: " +
                 p_gameObject->GetName());
  }
}
}  // namespace FalkonEngine