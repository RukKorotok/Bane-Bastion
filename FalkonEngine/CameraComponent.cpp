...
CameraComponent::CameraComponent(GameObject* gameObject)
  ...
  FE_CORE_ASSERT(
      m_transform != nullptr,
      "CameraComponent added to GameObject without TransformComponent!");
}
//-----------------------------------------------------------------------------------------------------------
...
void CameraComponent::Update(float deltaTime) {
  ...
  FE_CORE_ASSERT(m_transform != nullptr,
                 "Camera Transform lost during update!");
...
void CameraComponent::SetBaseResolution(int width, int height) {
  if (width <= 0 || height <= 0) {
    FE_CORE_ERROR("Invalid base resolution: " + std::to_string(width) + "x" +
                  std::to_string(height));
    return;
  }
  m_view->reset(sf::FloatRect(0.0f, 0.0f, static_cast<float>(width),
...
void CameraComponent::ZoomBy(float newZoom) {
  if (newZoom <= 0) {
    FE_CORE_WARN("Zoom factor must be positive. Provided: " +
