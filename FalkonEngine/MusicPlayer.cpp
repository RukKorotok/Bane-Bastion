...
void MusicPlayer::Play(const std::string& name, bool loop, float volume) {
  if (m_currentTrackName == name && m_currentMusic &&
      m_currentMusic->getStatus() == sf::Music::Playing) {
    return;
  }
  ...
  if (m_currentMusic) {
    ...
    FE_CORE_INFO("MusicPlayer: Playing track '" + name +
                 "' [Loop: " + (loop ? "Yes" : "No") +
                 ", Vol: " + std::to_string(volume) + "]");
  } else {
    FE_CORE_ERROR("MusicPlayer: Failed to play track '" + name +
                  "'. Not found in ResourceSystem!");
  }
}
//--------------------------------------------------------------------------------------------------------
...
void MusicPlayer::SetVolume(float volume) {
  float clampedVolume =
      (volume < 0.f) ? 0.f : (volume > 100.f ? 100.f : volume);
