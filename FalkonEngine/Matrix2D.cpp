...
Matrix2D::Matrix2D(Vector2Df position, float angle, Vector2Df scale) {
  ...
  FE_APP_TRACE("Matrix2D: Transform matrix created for position (" +
...
Matrix2D Matrix2D::operator*(const Matrix2D& other) const {
  ...
  for (int row = 0; row < 3; row++) {
    for (int col = 0; col < 3; col++) {
      result.m_matrix[row][col] = m_matrix[row][0] * other.m_matrix[0][col] +
...
Matrix2D Matrix2D::GetInversed() const {
  ...
  res.m_matrix[1][1] =
      ...
      invDet;
  res.m_matrix[1][2] =
      (m_matrix[1][0] * m_matrix[0][2] - m_matrix[0][0] * m_matrix[1][2]) *
      invDet;
  ...
  res.m_matrix[2][0] =
      ...
      invDet;
  res.m_matrix[2][1] =
      (m_matrix[2][0] * m_matrix[0][1] - m_matrix[0][0] * m_matrix[2][1]) *
      invDet;
  res.m_matrix[2][2] =
      (m_matrix[0][0] * m_matrix[1][1] - m_matrix[1][0] * m_matrix[0][1]) *
      invDet;
