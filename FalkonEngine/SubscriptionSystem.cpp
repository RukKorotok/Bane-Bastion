...
Observer::~Observer() {
  ...
  while (m_head) {
    ...
    FE_CORE_ASSERT(
        m_head->subject != nullptr,
        "SubscriptionNode has null subject during Observer destruction!");
...
void Observer::_Internal_Detach(Details::SubscriptionNode* node) {
  FE_CORE_ASSERT(node != nullptr,
                 "Attempted to detach a null node in Observer.");
...
Observable::~Observable() {
  ...
  while (m_head) {
    ...
    if (m_head->observer) {
      ...
      FE_CORE_WARN(
          "SubscriptionNode has null observer during Observable destruction.");
    }
...
void Observable::Notify(const GameEvent& event) {
  ...
  while (curr) {
    ...
    if (curr->observer) {
      ...
      FE_CORE_ERROR(
          "Broken SubscriptionNode detected during Notify: Observer is null!");
    }
    curr = next;
  }
  // For deep debug
  //  FE_APP_TRACE("Event notified to " + std::to_string(notifyCount) + "
  //  observers.");
}
//--------------------------------------------------------------------------------------------------------
void Observable::_Internal_Detach(Details::SubscriptionNode* node) {
  FE_CORE_ASSERT(node != nullptr,
                 "Attempted to detach a null node in Observable.");
