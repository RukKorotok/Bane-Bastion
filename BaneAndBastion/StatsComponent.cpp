...
// --------------------------------------------------------------------------------------------------------
void StatsComponent::InitStats(
    std::initializer_list<std::pair<StatType, float>> stats) {
...
void StatsComponent::SetStat(StatType type, float value) {
  // Optimization: prevent redundant updates and event broadcasting if the value
  // hasn't changed
  ...
  try {
    // Create and dispatch a GameEvent to notify listeners (UI, Health system,
    // etc.)
...
float StatsComponent::ChangeStat(StatType type, float deltaValue) {
  // Wrapper for relative modification; retrieves current value before applying
  // delta
