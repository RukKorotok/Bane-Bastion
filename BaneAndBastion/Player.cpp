...
// Player
// -----------------------------------------------------------------------------------------------------------
Player::Player(FalkonEngine::Vector2Df position, const std::string& name,
  ...
  if (playerInput) {
    // Subscribe the Player to input events to translate key presses into
    // movement/actions
  ...
  switch (event.type) {
    case FalkonEngine::GameEventType::PositionChanged: {
      ...
      if (p_gridManager) {
        ...
        if (!p_gridManager->CheckGridCollision(boundsX,
        ...
        if (!p_gridManager->CheckGridCollision(boundsY,
                                               GetCollisionCategory())) {
