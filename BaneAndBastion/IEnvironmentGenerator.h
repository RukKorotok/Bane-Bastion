...
class IEnvironmentGenerator {
 public:
  ...
  /**
   * @brief Pure virtual method to generate environment objects for a specific
   * chunk.
   * * @param cx Chunk X coordinate.
   * @param cy Chunk Y coordinate.
   * @param safeGridPos Global grid position to be kept clear of obstacles
   * (e.g., spawn point).
   * @param safeRadiusCells Square radius in cells around the safe position to
   * ignore generation.
   * @return A vector of pointers to the generated EnvironmentObjects.
   */
  virtual std::vector<EnvironmentObject*> Generate(
      int cx, int cy, FalkonEngine::Vector2Di safeGridPos = {-999, -999},
      int safeRadiusCells = 0) = 0;

 protected:
  /**
   * @brief Registers an asset type for the generation pool.
   * * @tparam T The class of the environment object (must derive from
   * EnvironmentObject).
   ...
   * @return type_index of the selected asset or typeid(void) if no asset was
   * chosen.
   */
  ...
  /// Map of asset types to their respective factory functions for
  /// instantiation.
  std::map<std::type_index, std::function<EnvironmentObject*(float, float)>>
      m_factories;
};
}  // namespace BaneAndBastion