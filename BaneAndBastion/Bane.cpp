...
Bane::Bane(FalkonEngine::Vector2Df pos, std::shared_ptr<IChainAnchor> anchor)
  ...
  // 2. Setup Physics: Configure Rigidbody with lower damping for "slippery"
  // movement
  ...
  // 3. Setup Stats: Bane has higher strength but the same base health as the
  // anchor
...
void Bane::OnNotify(const FalkonEngine::GameEvent& event) {
  switch (event.type) {
    case FalkonEngine::GameEventType::MouseMoved: {
      ...
      // Validate if the intended mouse target is within the allowed chain
      // radius
      if (!isMovementAllowed(event.direction)) break;

      FalkonEngine::Vector2Df mousePos = event.direction;
      FalkonEngine::Vector2Df currentPos =
      ...
      float distSq = std::pow(mousePos.x - currentPos.x, 2) +
                     std::pow(mousePos.y - currentPos.y, 2);
      if (distSq < 40.0f) {
        movement->Stop();
        break;
      }
      ...
      float deltaTargetSq = std::pow(mousePos.x - m_lastRawTarget.x, 2) +
                            std::pow(mousePos.y - m_lastRawTarget.y, 2);
    ...
    case FalkonEngine::GameEventType::PositionChanged: {
      // Forward movement delta to base Player class for grid-based collision
      // checks
...
bool Bane::isMovementAllowed(const FalkonEngine::Vector2Df& targetPos) const {
  ...
  // Allow movement only if target is within the allowed spherical/circular
  // constraint
...
void Bane::applyChainConstraint() {
  ...
  // "Leash" Logic: If current position exceeds max distance, pull Bane back to
  // the constraint edge
