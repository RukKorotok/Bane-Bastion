...
class IPathfindingStrategy {
 public:
   ...
   * @param collision The collision category of the agent (to determine passable
   * areas).
   * @return A vector of world coordinates representing the path from start to
   * target.
   */
  virtual std::vector<FalkonEngine::Vector2Df> GetPath(
      FalkonEngine::Vector2Df start, FalkonEngine::Vector2Df target,
      FalkonEngine::CollisionCategory collision) = 0;
};
}  // namespace BaneAndBastion