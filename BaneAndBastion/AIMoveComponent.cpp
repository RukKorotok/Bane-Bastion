...
// AIMoveComponent
// -----------------------------------------------------------------------------------------------------------
AIMoveComponent::AIMoveComponent(FalkonEngine::GameObject* gameObject)
...
// -----------------------------------------------------------------------------------------------------------
void AIMoveComponent::MoveTowards(FalkonEngine::Vector2Df start,
...
void AIMoveComponent::CalculateDesiredDir(float dt) {
  ...
  FalkonEngine::Vector2Df toTarget = {m_targetPos.x - m_startPos.x,
                                      m_targetPos.y - m_startPos.y};

  // Use squared distance (x^2 + y^2) for optimization to avoid expensive sqrt()
  // calls
  ...
  if (distSq > 4.0f) {
    ...
    // Integrate current velocity into m_startPos for a smooth reference point
    // update
    ...
    FE_APP_TRACE("AIMoveComponent: Target reached by entity " +
                 std::to_string(ev.entityID));
  }
}
}  // namespace BaneAndBastion