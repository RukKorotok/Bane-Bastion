...
FalkonEngine::Vector2Df Bastion::GetAnchorPosition() const {
  // Retrieves world coordinates for secondary systems (like the chain/tether
  // system)
  auto transform =
...
// --------------------------------------------------------------------------------------------------------
float Bastion::GetMaxChainDistance() const { return m_maxChainDistance; }
...
void Bastion::OnNotify(const FalkonEngine::GameEvent& event) {
  ...
  switch (event.type) {
    ...
    case FalkonEngine::GameEventType::PositionChanged: {
      // Relay movement updates to other systems (e.g., the chain system or
      // triggers)
