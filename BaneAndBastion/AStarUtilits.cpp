...
//--------------------------------------------------------------------------------------------------------
std::vector<FalkonEngine::Vector2Di> AStarUtils::FindPath(
  ...
  // Priority queue to store nodes to explore. Lowest f cost (g + h) is always
  // on top.
  ...
  const int MAX_ITERATIONS =
      500;  // Performance safety cap to prevent freeze in large maps
  ...
  while (!openSet.empty() && iterations < MAX_ITERATIONS) {
    ...
    // Goal check: if reached, reconstruct the path by traversing parents back
    // to start
    if (current->pos == target) {
      ...
      std::reverse(path.begin(),
                   path.end());  // Reverse to get Start -> Target order
      cleanup();
      return path;
    }
    ...
    for (int i = 0; i < 8; ++i) {
      ...
      // ACCESSIBILITY CHECK
      // Skip if cell is out of bounds or blocked for the seeker's category
      // (using bitmask logic)
      ...
      // When moving diagonally, ensure both adjacent orthogonal cells are also
      // passable. This prevents the agent from glitching through thin wall
      // corners.
      if (i >= 4) {
        auto side1 =
            gm->GetCell(current->pos.x + neighbors[i].x, current->pos.y);
        auto side2 =
            gm->GetCell(current->pos.x, current->pos.y + neighbors[i].y);

        // Если хоть одна из боковых ячеек непроходима для нас - диагональ
        // закрыта
        if (!side1 || side1->Blocks(seekerCategory) || !side2 ||
            side2->Blocks(seekerCategory)) {
          continue;
        }
      }
      ...
      if (it == allNodes.end()) {
        ...
        // Found a shorter path to an already visited location: update costs and
        // parent
        it->second->g = newG;
