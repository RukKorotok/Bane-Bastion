...
GridManager::GridManager(std::unique_ptr<IEnvironmentGenerator> generator,
                         std::unique_ptr<NPCSpawner> spawner)
    : m_generator(std::move(generator)), m_npcSpawner(std::move(spawner)) {
  FE_CORE_ASSERT(m_generator != nullptr,
                 "GridManager created with null generator!");
  ...
  if (activeScene && activeScene->GetWorld()) {
    ...
    FE_CORE_INFO(
        "GridManager: Successfully initialized and subscribed to World.");
  } else {
    FE_CORE_ERROR(
        "GridManager: Failed to subscribe to World. Active scene or world is "
        "null!");
  }
}
...
void GridManager::UpdateVisibleArea(const FalkonEngine::Vector2Df& playerPos,
                                    int viewDistance) {
  ...
  for (int x = center.x - viewDistance; x <= center.x + viewDistance; ++x) {
    for (int y = center.y - viewDistance; y <= center.y + viewDistance; ++y) {
      ...
      if (m_chunks.find(pos) == m_chunks.end()) {
        FE_APP_TRACE("GridManager: Generating new chunk at [" +
        ...
        try {
          ...
          auto envObjects =
              m_generator->Generate(pos.x, pos.y, safePos, safeRadius);
          ...
          if (auto* scene =
            ...
            for (auto* env : envObjects) {
              ...
              auto objPos =
                  env->GetGameObject()
                      ->GetComponent<FalkonEngine::TransformComponent>()
                      ->GetWorldPosition();
              int gx = WorldToGrid(objPos.x);
              int gy = WorldToGrid(objPos.y);
              FalkonEngine::Vector2Di lp = WorldToLocal(gx, gy);

              m_chunks[pos].cells[lp.x][lp.y].blockingMask |=
                  static_cast<std::uint8_t>(env->GetCollisionCategory());
            }
          ...
          FE_CORE_ERROR("GridManager: Failed to generate chunk. Error: " +
                        std::string(e.what()));
        }
      }
    }
...
void GridManager::ChangeGenerator(
    std::unique_ptr<IEnvironmentGenerator> newGen) {
  if (newGen) {
...
bool GridManager::CheckGridCollision(const sf::FloatRect& bounds,
...
void GridManager::OccupyArea(const FalkonEngine::Vector2Df& worldPos,
                             const FalkonEngine::Vector2Df& size,
                             FalkonEngine::CollisionCategory category) {
...
void GridManager::UnregisterEntity(const FalkonEngine::Vector2Df& pos,
                                   const FalkonEngine::Vector2Df& size,
                                   FalkonEngine::CollisionCategory category) {
...
FalkonEngine::Vector2Df GridManager::GetNearestPassablePoint(
