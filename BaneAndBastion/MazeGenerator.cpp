...
std::vector<EnvironmentObject*> MazeGenerator::Generate(
    int cx, int cy, FalkonEngine::Vector2Di safeGridPos, int safeRadiusCells) {
  ...
  while (!stack.empty()) {
    ...
    if (!neighbors.empty()) {
      ...
      auto next = neighbors[std::uniform_int_distribution<int>(
          0, (int)neighbors.size() - 1)(gen)];
  ...
  for (int x = 0; x < size; ++x) {
    for (int y = 0; y < size; ++y) {
      float wx = (cx * size + x) * GameSettings::PixelsPerUnit +
                 GameSettings::PixelsPerUnit * 0.5f;
      float wy = (cy * size + y) * GameSettings::PixelsPerUnit +
                 GameSettings::PixelsPerUnit * 0.5f;
