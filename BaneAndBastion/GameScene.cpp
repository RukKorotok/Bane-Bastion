...
GameScene::GameScene(const std::string& name)
    : Scene(name), m_gridManager(nullptr) {}
...
void GameScene::Start() {
  ...
  float centerX =
      (GameSettings::ChunkSize / 2.0f) * GameSettings::PixelsPerUnit +
  ...
  try {
    ...
    m_bane = std::make_shared<Bane>(
        FalkonEngine::Vector2Df(centerX + 50.0f, centerY + 50.0f), m_bastion);
...
// -----------------------------------------------------------------------------------------------------------
void GameScene::AddToChunk(FalkonEngine::Vector2Di chunkPos,
                           FalkonEngine::Actor* obj) {
  if (!obj) return;
...
void GameScene::ClearChunk(FalkonEngine::Vector2Di chunkPos) {
  ...
  if (it != m_chunkContent.end()) {
    FE_APP_TRACE("Clearing chunk at [" + std::to_string(chunkPos.x) + ", " +
