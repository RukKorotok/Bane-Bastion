...
// DynamicActor
//-----------------------------------------------------------------------------------------------------------
DynamicActor::DynamicActor(FalkonEngine::Vector2Df position,
  ...
  if (auto* gameScene = dynamic_cast<GameScene*>(activeScene)) {
    ...
    FE_CORE_ERROR(
        "DynamicActor: Active scene is null or not a GameScene! Actor '" +
        name + "' will not have a GridManager.");
    p_gridManager = nullptr;
  }
  auto playerRenderer = p_gameObject->GetComponent<SpriteRendererComponent>();
  playerRenderer->SetPixelSize(GameSettings::PixelsPerUnit * 0.5f,
                               GameSettings::PixelsPerUnit * 0.5f);

  FE_APP_TRACE("DynamicActor: '" + name + "' initialized at position (" +
