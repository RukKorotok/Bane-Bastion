...
class BaneMoveComponent : public MoveComponent {
 public:
  ...
  /**
   * @brief Immediately halts the entity and deactivates the current movement
   * task. Overrides the base MoveComponent behavior.
   */
  void Stop() override;

  /**
   * @brief Calculates the steering direction based on the current target
   * vector. Logic is executed if m_isActive is set to true.
 ...
 private:
  ...
  FalkonEngine::Vector2Df
      m_startPose;          ///< Cached starting position for current segment.
  bool m_isActive = false;  ///< Flag to enable/disable automated steering.

  /**
   * @brief Tolerance threshold (squared distance) for arriving at the target
   * point. Prevents "jittering" behavior when the entity is very close to the
   * goal.
   */
