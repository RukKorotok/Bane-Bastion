...
std::vector<NPC*> NPCSpawner::GenerateNPCs(const FalkonEngine::Vector2Di& cp,
                                           const Chunk& chunk) {
  ...
  for (int lx = 0; lx < GameSettings::ChunkSize; ++lx) {
    for (int ly = 0; ly < GameSettings::ChunkSize; ++ly) {
      ...
      float wx =
          (cp.x * GameSettings::ChunkSize + lx) * GameSettings::PixelsPerUnit +
          GameSettings::PixelsPerUnit * 0.5f;
      float wy =
          (cp.y * GameSettings::ChunkSize + ly) * GameSettings::PixelsPerUnit +
          GameSettings::PixelsPerUnit * 0.5f;

      // INITIALIZATION: Create a prototype on the first ever attempt to spawn
      // this type
      ...
      if (entry.prototype != nullptr) {
        ...
        if (!chunk.cells[lx][ly].Blocks(entry.category)) {
          ...
          FE_APP_TRACE("Prototype deployed to world at: " + std::to_string(wx) +
                       ", " + std::to_string(wy));
        }
        continue;  // Skip factory call since we were trying to place the
                   // prototype
      }
