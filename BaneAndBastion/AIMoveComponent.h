...
class AIMoveComponent : public MoveComponent {
 public:
  ...
  /**
   * @brief Immediately halts the entity and deactivates any active path
   * following.
   */
  void Stop() override;

  /**
   * @brief Initiates movement from a starting point towards a world-space
   * target.
   * @param start Initial position for the movement segment.
   * @param target Destination point to reach.
   */
  void MoveTowards(FalkonEngine::Vector2Df start,
 ...
 private:
  FalkonEngine::Vector2Df m_startPos{
      0, 0};  ///< Current segment starting position.
  FalkonEngine::Vector2Df m_targetPos{
      0, 0};  ///< Target destination in world coordinates.
  bool m_active =
      false;  ///< Whether the component is actively steering towards a target.
};
}  // namespace BaneAndBastion