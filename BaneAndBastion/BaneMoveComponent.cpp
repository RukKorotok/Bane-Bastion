...
void BaneMoveComponent::LaunchToTarget(
    const FalkonEngine::Vector2Df& startPos,
...
void BaneMoveComponent::CalculateDesiredDir(float dt) {
  // If the component is inactive, only handle friction and position
  // synchronization
  if (!m_isActive) {
    ApplyFriction(dt);
    // Synchronize virtual position even when stationary to account for residual
    // velocity
  ...
  if (distSq > m_arrivalThresholdSq) {
    ...
    float currentSpeed = std::sqrt(m_currentVelocity.x * m_currentVelocity.x +
...
void BaneMoveComponent::ApplyFriction(float dt) {
  ...
  float speed = std::sqrt(m_currentVelocity.x * m_currentVelocity.x +
                          m_currentVelocity.y * m_currentVelocity.y);
