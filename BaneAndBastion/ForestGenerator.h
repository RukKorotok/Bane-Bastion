 ...
 * chunks with trees, walls, and other forest assets using deterministic
 * randomness.
 */
class ForestGenerator : public IEnvironmentGenerator {
 public:
   ...
   * @param safeGridPos Global grid position that should be kept clear of
   * obstacles.
   * @param safeRadiusCells Square radius around the safe position to ignore
   * generation.
  ...
  virtual std::vector<EnvironmentObject*> Generate(
      ...
      int safeRadiusCells = 0) override;
};
}  // namespace BaneAndBastion