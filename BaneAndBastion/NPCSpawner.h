 ...
 */
struct SpawnEntry {
  std::function<NPC*(float, float)>
      factory;   ///< Function to create a new NPC instance.
  float weight;  ///< Probability weight for spawning.
  FalkonEngine::CollisionCategory
      category;                ///< Collision category assigned to the NPC.
...
class NPCSpawner {
 public:
   ...
   */
  virtual std::vector<NPC*> GenerateNPCs(const FalkonEngine::Vector2Di& cp,
                                         const Chunk& chunk);
 ...
 protected:
  ...
  void RegisterAsset(float weight) {
    ...
    m_registry[idx] = {[](float x, float y) -> NPC* {
                         return new T(FalkonEngine::Vector2Df(x, y));
                       },
                       weight, FalkonEngine::CollisionCategory::None, nullptr,
                       false};
  }

 private:
  /**
   * @brief Selects a random NPC type index from the registry using a
   * distribution.
